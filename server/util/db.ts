...
export const populateDatabase = async () => {
  ...
  if (process.env.NODE_ENV === 'test'){
    ...
    console.log('populating testDB');    
  }else{
    
    try {
      const stations = await parseStations(
        '../files/Helsingin_ja_Espoon_kaupunkipy%C3%B6r%C3%A4asemat_avoin.csv'
      );
      ...
      await Journey.bulkCreate(
        await parseJourneys('../files/2021-05.csv', stationIds)
      );  

      console.log('parsing journeys 2/3');
      await Journey.bulkCreate(
        await parseJourneys('../files/2021-06.csv', stationIds)
      );  

      console.log('parsing journeys 3/3');
      await Journey.bulkCreate(
        await parseJourneys('../files/2021-07.csv', stationIds)
      );  
